...
const Summary = () => {
    const fakedata = [];
    const modules =['30D', '60D', '180D', '1Y', '2Y', '3Y']
    ...
    async function LoadData() {
        let user: any = await supabase.from('users').select();
    ...
    setIsAdmin(userData1.data[0]['role'] != null)
    if (userData1.data[0]['role'] == null){
...
LoadData();
    ...
    return (
        <section className="mx-15 flex flex-col mt-30 justify-center items-center">
           ...
           <div className="flex flex-row">
{
    ...
    <div className={`p-3 text-white  font-poppins text-center px-7 py-5 font-bold text-2xl ${e=='30D' ? 'bg-red-400' : e=='60D' ? 'bg-orange-800' : 
...
{e} <br/> Module
    ...
    {
        modules2.map((item) =>
     <div className={`p-3 text-white font-poppins text-center rounded-4xl mt-4 px-7  font-bold text-xl ${e=='30D' ? 'bg-red-400' : e=='60D' ? 'bg-orange-800' : 
e == '180D' ? 'bg-orange-500' : e == '1Y' ? 'bg-yellow-500' : e == '2Y' ? 'bg-green-400' :
'bg-cyan-400'
